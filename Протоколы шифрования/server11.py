...
def check_key():
    ...
    print(f"[server] Сгенерированные закрытый и открытый ключи")
    conn.send(pickle.dumps(B))
    msg = conn.recv(1024)
    if decrypt(pickle.loads(msg), K, A) != SERVER_MESSAGE_HELLO:
        print("[server] Неправильный клиентский ключ. Закрывающее соединение...")
        conn.close()
    else:
        print("[server] Успешная проверка ключа. Подключение...")
        start_serving()
